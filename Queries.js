...
db.getCollection('restaurant').find({}, {restaurante_id: 1, name: 1, borough: 1, cuisine: 1, _id: 0})
...
db.restaurant.find({"borough": { "$ne": "Brooklyn" },"cuisine": { "$not": { "$eq": "American" } },"grades.grade": "A"}).sort({ "cuisine": -1 })
...
db.restaurant.find({ "grades.grade": "A", "grades.score": 11, "grades.date": ISODate("2014-08-11T00:00:00Z") },{ "restaurant_id": 1, "name": 1, "grades": 1, "_id": 0  })
...
db.restaurant.find({ "address.coord.1": { $gt: 42, $lte: 52 }},  {"restaurant_id": 1,"name": 1,"address.street": 1,"address.coord": 1,  "_id": 0  })
